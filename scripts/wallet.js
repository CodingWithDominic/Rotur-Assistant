...
const notooltip = await new Promise(resolve =>
    chrome.storage.local.get('wallet_notooltip', data => resolve(data.wallet_notooltip || false))
) ?? false;

let bal_cache = 0;
let suminprogress = false

import { sanitize, formatDate, openSuccessPopup, MiniError, FixDecimal } from "../index.js"
...
async function getWaitTime() {
    const waitdata = await fetch(`https://api.rotur.dev/claim_time`, {headers: authform}).then(res => res.json())
...
async function getTransactionHistory(accdata) {
    ...
    for (let i=0; i<transactiondata.length; i++) {
        ...
        transactioncard.querySelector('.amtinvolved').textContent = `Amount involved: ${FixDecimal(transaction_snippet.amount)}`;
    ...
    document.getElementById('averageprofit').innerText = `Recent average profit margin: ${FixDecimal(profit / (transactiondata.length - 1))}`
...
async function getEconomicData(currentaccdata) {
    ...
    const stats = (stats_cache1 ? stats_cache1 : await fetch('https://api.rotur.dev/v2/stats/economy').then(res => res.json()))
    ...
    document.getElementById('statslist').setHTML(`
        ...
        <hr class="full-size dotted_separator">
        <h2>Your Stats</h2>
...
async function sumBalances() {
    ...
    async function PerformSum(i) {
        let accdata = {}
        let accjson = {}
        accdata = (accounts[i].uuid == activeacc.uuid) ? activeaccdata : await fetch(`https://devfund.rotur.dev/api/me`, {headers: {"Authorization": `Bearer ${accounts[i].token}`}}).then(res => res.json())
        const gain = (accdata['sys.currency'] ?? accdata.balance ?? 0)
        sum_cache[0] += gain
        accjson[accounts[i].name] = gain
        sumcache2.push(accjson)
        flat_cache[accounts[i].name] = gain
    }
    const promises = []
    ...
    for (let j=0; j < accounts.length; j++) {
        ...
        sumcard.querySelector('a').href = `lookup.html?user=${(accounts[j].name ?? "Spectator")}`
...
async function walletpage() {
    ...
    if (accounts.length == 0) {
        ...
        <h1>Your Wallet</h1>
        <hr class="full-size">
        <h3>You are not signed in! Please head over to the <a href="accounts.html">account manager</a> to add an account first.</h3>
        `, {sanitizer: sanitizer})
        return;
    }
    ...
    activeaccdata = await fetch(`https://api.rotur.dev/v2/me`, {headers: authform}).then(res => res.json()).catch(err => {
        document.getElementsByClassName('container')[0].setHTML(`
            <h1>Your Wallet</h1>
            <hr class="full-size">
            <h3>A communication error has occurred. If you're sure it's not your connection, then Rotur may be down right now.</h3>
    ...
    if (activeaccdata['sys.currency'] == undefined) {
        ...
        <h1>Your Wallet</h1>
        <hr class="full-size">
        <h3>If you see this, you were probably rate-limited by the Rotur API. Please wait a bit, then try again.</h3>
        `, {sanitizer: sanitizer})
        return;
    }
    if (accounts.length > 1) {
        if (sum_cache && (Date.now() - sum_cache[1] < 300000)) {
            document.getElementById('totalsumofaccs').textContent = `Total Balance (Sum of all accounts): ${sum_cache[0]}` // Refresh sum cache after 5 minutes have passed
    ...
    ${accounts.length == 1 ? `` : `<h2 id='totalsumofaccs'> Total Balance (Sum of all accounts): ${suminprogress ? `...` : sum_cache[0] ?? `...`}</h2>`}
    `, {sanitizer: sanitizer})
    if ((systemowner.toLowerCase() != activeacc.name.toLowerCase()) && !notooltip) {
        ...
        <div id="fineprintactionbar">
            <label class="labelandinput">
            <input type="checkbox" id="dontshowtooltipagain">
            Don't show this tooltip again
            </label>
            <button id="dismisstooltip">Dismiss</button>
        </div>
        `, {sanitizer: sanitizer})
        document.getElementById('claimfineprint').style.display = "block"
    }
...
document.addEventListener('click', async function(e) {
    switch (e.target.id) {
        ...
        case ('dismisstooltip'): {
            if (document.getElementById('dontshowtooltipagain').checked) {
                await chrome.storage.local.set({wallet_notooltip: true})
            }
            document.getElementById('claimfineprint').remove()
            break;
        }
        case ('dailycreditnotif'): {
        ...
        case ('transactionreload'): {
            document.getElementById('transactionreload').textContent = '…'
            document.getElementById('transactionreload').disabled = true
            transaction_cache = await fetch(`https://api.rotur.dev/v2/me`, {headers: authform}).then(res => res.json())
        ...
        case ('statsreload'): {
            ...
            stats_cache1 = await fetch('https://api.rotur.dev/v2/stats/economy').then(res => res.json())
        ...
        case ('dailyclaim'): {
            ...
            if (activeaccdata2.currency <= 1000) {
                if ((activeaccdata2.currency > 500) && (e.target.id == 'dailyclaim')) {
                    ...
                    var dailysuccess = await fetch(`https://api.rotur.dev/claim_daily`, {headers: authform}).then(res => res.json())
                }
            } else {
        ...
        case ('sendcredits'): {
            ...
            if (document.getElementById('transferuser').value == "") {
                ...
                recipientdata = await fetch(`https://api.rotur.dev/profile?name=${document.getElementById('transferuser').value}&include_posts=no`).then(res => res.json())
                if (recipientdata.error) {
                    potentialerrormsg = `This user does not exist`
                } else if (recipientdata.id == activeacc.uuid) {
        ...
        default: {
            if (e.target.className == 'tab') {
                ...
                document.getElementById('statspanel').style.display = (e.target.id == 'statstab') ? 'block' : 'none'
...
if (activeacc.uuid && !flagged.includes(activeacc.uuid)) {
    document.getElementById('compareusersform').addEventListener('submit', async function(e) {
        e.preventDefault()
        const user1 = document.getElementById('compareuser1').value || activeacc.name
        const user2 = document.getElementById('compareuser2').value || activeacc.name

        const user1promise = fetch(`https://api.rotur.dev/profile?name=${user1}`).then(res => res.json())
        const user2promise = (user1.toLowerCase() === user2.toLowerCase()) ? user1promise : fetch(`https://api.rotur.dev/profile?name=${user2}`).then(res => res.json()) // Slight optimization in case user 1 is the same as user 2. Just reuse user 1's data rather than doing the fetch again
        const statspromise = fetch('https://api.rotur.dev/v2/stats/economy').then(res => res.json())

        const [user1data, user2data, stats] = await Promise.all([user1promise, user2promise, statspromise])

        if (user1data.error || user2data.error) {
            if (user1data.error && user2data.error) {
                document.getElementById('compareerror').replaceChildren(MiniError("failure", "Both users entered do not exist"))
            } else if (user1data.error) {
                document.getElementById('compareerror').replaceChildren(MiniError("failure", "User 1 does not exist"))
            } else if (user2data.error) {
                document.getElementById('compareerror').replaceChildren(MiniError("failure", "User 2 does not exist")) // Over-engineering at its finest
            }
            setTimeout(() => {
                document.getElementById('compareerror').replaceChildren()
            }, 10000)
            return;
        }

        const user1bal = FixDecimal(user1data.currency ?? 0)
        const user2bal = FixDecimal(user2data.currency ?? 0)

        const majoruser = (user1bal > user2bal) ? user1 : user2
        const minoruser = (user1bal > user2bal) ? user2 : user1
        const majorbal = (user1bal > user2bal) ? user1bal : user2bal
        const minorbal = (user1bal > user2bal) ? user2bal : user1bal
        const baldiff = FixDecimal(majorbal - minorbal)
        let multiplier = majorbal / minorbal
        ...
        document.getElementById('compareusersdata').setHTML(`
            <table class="compareuserstable">
                <tbody>
                    <tr>
                        <td>Users</td>
                        <td><a href="lookup.html?user=${sanitize(user1)}">${sanitize(user1)}</a></td>
                        <td><a href="lookup.html?user=${sanitize(user2)}">${sanitize(user2)}</a></td>
                    </tr>
                    <tr>
                        <td>Balance</td>
                        <td>${FixDecimal(user1bal)}</td>
                        <td>${FixDecimal(user2bal)}</td>
                    </tr>
                    <tr>
                        <td>Economy %</td>
                        <td>${((user1bal * 100) / stats.total).toFixed(2)}%</td>
                        <td>${((user2bal * 100) / stats.total).toFixed(2)}%</td>
                    </tr>
                    <tr>
                        <td>Compared to average balance</td>
                        <td>${(user1bal / stats.average).toFixed(2)}&times; average</td>
                        <td>${(user2bal / stats.average).toFixed(2)}&times; average</td>
                    </tr>
                    <tr>
                        <td>Real-world equivalence ($)</td>
                        <td>$${(user1bal * (cents / 100)).toFixed(2)}</td>
                        <td>$${(user2bal * (cents / 100)).toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td>Real-world equivalence (£)</td>
                        <td>£${(user1bal * (pence / 100)).toFixed(2)}</td>
                        <td>£${(user2bal * (pence / 100)).toFixed(2)}</td>
                    </tr>
                </tbody>
            </table>
            <table class="compareuserstable">
                <tbody>
                    ...
                    <tr>
                        <td>Balance</td>
                        <td>${FixDecimal(user1bal + user2bal)}</td>
                    </tr>
                    <tr>
                        <td>Economy %</td>
                        <td>${(((user1bal + user2bal) * 100) / stats.total).toFixed(2)}%</td>
                    </tr>
                    <tr>
                        <td>Compared to average balance</td>
                        <td>${((user1bal + user2bal) / stats.average).toFixed(2)}&times; average</td>
                    </tr>
                    <tr>
                        <td>Real-world equivalence ($)</td>
                        <td>$${((user1bal + user2bal) * (cents / 100)).toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td>Real-world equivalence (£)</td>
                        <td>£${((user1bal + user2bal) * (pence / 100)).toFixed(2)}</td>
                    </tr>
                </tbody>
            </table>
            <p>${(user1bal === user2bal) ? `${sanitize(majoruser)} has the same amount of credits as ${sanitize(minoruser)} (${user1bal})` : `${sanitize(majoruser)} has ${baldiff} more credits than ${sanitize(minoruser)}`}</p>
            ${(user1bal !== user2bal) ? `<p>${sanitize(majoruser)} has ${FixDecimal(multiplier)}&times; more credits than ${sanitize(minoruser)}</p>` : ``}
        `, {sanitizer: sanitizer})
    })
}